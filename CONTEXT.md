...
| C5 | **本仓库不是 git 仓库** | 没有 `git checkout` 可回滚，改文件前想清楚 |
...
| `docs/迭代计划.md` | ★ **交付节奏**：竖切 I0~I5、每个迭代的 DoD 四件套、B7 棘轮燃尽表、不采用的敏捷做法。写明「待决」的地方**没有**替你做决定 |
...
7. **不要用终端整文件改写源文件**：PS 5.1 按 GBK 落盘会把中文注释变乱码，而本仓库（C5）
   没有 git 可回滚。改代码一律用编辑工具。
...
12. **报绿前先自报分母**：`n passed` 只是分子。某一段整块停止执行只会让数字变小、不会变红，
