...
* ⚠️ 此前这里还写「门禁会强制『代码里每个类要么对得上契约块、要么登记过』，所以这里不可能
  偷偷多出一个没人管的类型」——**这句话当时是假的**（2026-09-25 实测）：实现侧 144 个类里
  **29 个**既没有契约声明、也不在 `manifest.classes` / `impl_only` / `CONTRACT_FIRST` 里，
  当时的 13 个门禁照样全绿（随后 14 个也一样 —— 新加的 `enum-members` 只管**枚举成员**，
  管不到类清单）。本文件里的 `StrategyHandle`（契约 §2.1.1 只在散文里点名了两个字段）
  就是其中一个。原因：`verify_contract_signature.py` 只查「登记过的成员还在不在」，
  `verify_contract_appendix.py` 只审主契约**附录 G 节**。
  **2026-09-25 起这句话由判据兜底**：新门禁 `class-coverage`（`tools/verify_class_coverage.py`）
  要求每个类落进「契约声明 / 登记表 / `DEBT` / `LOCAL`」四集合之一。上面那 29 个已逐个落表
  （7 个 `DEBT` + 22 个 `LOCAL`；`StrategyHandle` 在 `DEBT` 那一栏，理由写在表里），
  漏登记一个即报 `CC-UNCOVERED`。**但它并不证明实现对** —— 只证明「有人认领过这个类」；
  而且**提及不算覆盖**（29 个里 13 个只在契约散文里出现过）。
  计数更正：此处先前写「145 个类」——2026-09-25
