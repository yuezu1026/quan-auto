...
| 前提 | 现状 | 依据 |
| --- | --- | --- |
| 每个迭代**可运行** | **成立**（2026-09-24 I2 S3 收工后）：`.venv` + pytest 可跑（`104 passed`）、`pyproject.toml`、`quanauto/` 包、`tests/`；依赖清单**不再为空**（`dependencies = ["pandas>=2.0"]`，使用者 `quanauto/datasources.py`；I0/I1 当时是空数组，因为那时的竖切只用标准库） | 本次实测，证据 `tools/gates-report.txt` |
...
| I2 | 把数据换成真实数据源 | AKShare/Baostock 适配器写入 PostgreSQL，回测改读真实日线 | 进行中 | `quanauto/datacenter.py`（S1：PIT / `as_of` 边界层）、`tools/verify_data_center_pit.py`、`tests/test_data_center_pit.py`、`quanauto/datasources.py`（S2：采集侧适配器）、`tools/verify_data_center_adapter.py`、`quanauto/pgstore.py`（S3：落库侧读写）、`tests/test_data_center_store.py` |
...
| 产物 | `AKShare` / `Baostock` 适配器写入 PostgreSQL；回测改读真实日线 | 端到端回测跑通，且 `dc_daily_bar` 有数据 | **S2 交付了采集侧适配器**：`quanauto/datasources.py`（三角色的源列名/取值映射 + 归一化 + `ValidationReport`）；**S3 前半交付了落库侧**：`quanauto/pgstore.py`（按 `data_version` 绑定的读 + 按业务主键的幂等 upsert + 驱动惰性适配）。**回测改读真实日线仍未开始**，这一行还不能打勾 |
...
**不能**证明 PostgreSQL 接受它；② 库端证据仍只有容器通道的 `tools/sql-smoke-report.txt` 与 `tools/falsify-report.txt`，
那两份只对 `postgres:17` 有效，且**改过任何 `db/*.sql` 之后即作废**。
