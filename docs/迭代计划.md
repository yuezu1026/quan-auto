...
- **I2a-1（本轮落地）**：取数失败**可分类**。`SourceAdapterError` 新增 `source`／`kind`／`retryable`
  ...
  **不降级成 `UNKNOWN`** —— 静默降级会让「表写错了」和「真没归类」长得一模一样。
  ...
  `--selftest`，干净合成样本仍绿）。
- **I2a-2（进行中，2026-09-24）**：`EastMoneyAdapter._default_fetch` 走标准库 `urllib` 接上东财端点。
  **不引入第三方 SDK**：该源无官方 Python SDK。
  本轮落地的是**与「接哪个 report」无关的那一半**：唯一网络出口 `_http_get_json`
  （`quanauto/datasources.py`，标准库 `urllib`，`opener` 可注入 ⇒ 离线可测），
  以及 `tools/eastmoney_transport_smoke.py`（人工执行、**不是门禁**，证据
  `tools/eastmoney-transport-smoke-report.txt`）。离线那一侧的用例全部打**本机** HTTP 服务：
  query 拼装 / UA / 状态码 429・401・403・503 / 非 JSON 正文 / 端口关闭，
  一路走到 `classify_source_failure` 的真实类别（真传输 → 真适配器 → 真分类器，**不碰外网**）。
- **本轮实测把 I2b 的「真实不确定性」提前引爆了**：`EASTMONEY_FINANCIAL` 的 9 列
  **没有任何单一 report 能喂满**。`RPT_LICO_FN_CPD` 给 5/9（缺 `REPORT_DATE`/`REPORT_TYPE`/
  ...
  `REPORT_TYPE`**（真实响应里只有 `REPORT_TYPE_CODE`），而 `EASTMONEY_REPORT_TYPE` 正是按
  **中文报表名**建的映射 ⇒ 该映射表在真实返回上**没有生产者**。⇒ 映射表**不是「未验证」，
  是「已部分证伪」**，登记在数据中心契约附录 **B12**。处置：**本切片不改映射表、不删列**
  （改表等于把产品决策吞进实现；删列会让 `dc_financial` 的科目永久为空），待裁决。
  这条只能验「接线形状」，**验不了**「取回来的数对不对」—— 那是 I2b。
