...
| 前提 | 现状 | 依据 |
| --- | --- | --- |
| 每个迭代**可运行** | **成立**（2026-09-23 I1 收工后）：`.venv` + pytest 可跑（`25 passed`）、`pyproject.toml`、`quanauto/` 包、`tests/`；依赖清单仍是空的（`dependencies = []`）—— 但这是**选择**不是缺口：I1 的竖切只用标准库就够 | 本次实测，证据 `tools/gates-report.txt` |
...
| I2 | 把数据换成真实数据源 | AKShare/Baostock 适配器写入 PostgreSQL，回测改读真实日线 | 进行中 | `quanauto/datacenter.py`（S1：PIT / `as_of` 边界层）、`tools/verify_data_center_pit.py`、`tests/test_data_center_pit.py` |
...
| 件 | 具体是什么 | 判定方式 | 证据 |
| --- | --- | --- | --- |
| 契约 | 数据中心的 9 个适配器契约 + `as_of_date` 语义（禁止绕过 `DataCenter.as_of()`） | 与数据中心契约 v1.0 逐条对齐 | `docs/智能量化交易平台-数据中心接口契约文档.md`（**附录 A**：S1 的 9 条实现侧裁决与偏差）、`tools/contract-signature-manifest.json`（`impl_only` 段） |
| 产物 | `AKShare` / `Baostock` 适配器写入 PostgreSQL；回测改读真实日线 | 端到端回测跑通，且 `dc_daily_bar` 有数据 | **S1 只交付了 PIT 边界层**：`quanauto/datacenter.py`；适配器与 PG 写入**仍未开始**，这一行还不能打勾 |
| 门禁 | 适配器归一化检查（源特有字段名不得出现在数据类上）+ PIT 一致性检查 | 接入 `run_all_gates.py` 且 tier-A 绿 | `tools/verify_data_center_pit.py`（PIT 那一半已注册且 tier-A 绿）、`tools/gates-report.txt`；**适配器归一化那一半尚未写** |
| 触发测试 | 构造一条 `available_date > as_of_date` 的数据，确认取数必须报错而不是静默返回 | 一次真实的拒绝记录 | `tests/test_data_center_pit.py` 的 `test_pit_guard_catches_leak_from_store_that_ignores_window`（store 无视查询窗口，取数被 `FutureDataAccessError` 拒绝） |
