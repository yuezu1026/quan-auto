...
| I4 | 绩效看板（由原「看板 + Java 平台层」收缩而来） | 读一份回测结果 → 展示绩效指标表与资金曲线 | 已交付 | `quanauto/dashboard.py`、`quanauto/cli.py`（含 dashboard 子命令）、`tests/test_dashboard.py`、`tools/verify_dashboard.py`、`tools/dashboard-failure-report.txt` |
...
**复权因子真的接上（HFQ）后**，处理方式是**删掉那条用例 + 销掉缺口**，**不是**把期望值改成正确值——
后者会把「缺口已关闭」和「期望值跟着实现跑」写得一模一样。

> **触发条件订正（2026-09-25 实测）**：这句原先写「`S3` 真正接上 HFQ 后」。S3 **已交付**，
> 而因子**仍未接上**（`quanauto/datacenter.py` 的 `get_adjustment_factor()` 仍 `return 1.0`，
> `dc_adjust_factor` 仍无采集路径）⇒ 拿「哪一步交付」当触发条件会被读成「S3 已交付 ⇒ 该删了」，
> 而那会删掉一条**仍然正确**的测试。故触发条件只留「因子真的接上」。订正记录见 DC 契约附录 A6。
...
- **门禁**：`tools/run_all_gates.py` → **12/12 PASS**、棘轮 **39/39**（现取，见 `tools/gates-report.txt`；本行 I2b-1 交付当时是 10/10；2026-09-25 B7 还债后又现取一次是 **13/13、棘轮 2/2**）。
...
- 🔴 **B20 重读（2026-09-25）**：上面那个「读回 86 行」与「库内 11 列一致」**都是同一条连接上的自读**，
  而当时那条连接已经被 `execute()` 留下了未结束的隐式事务 ⇒ `transaction()` 退化成 SAVEPOINT、**永不 COMMIT**。
  ”那 86 行真的入过库“这句话**不成立**（重演见契约附录 B20：同一次运行里同连接自读数到 3、新连接数到 0）。
  两个缺陷已修（`autocommit=True` + `description is None`），并有进仓守卫；未修的旧证据不要再当作持久化。
...
- **判据**：`python -m pytest -q` → **273 passed**（2026-09-25 现取，B20 修后 +3）；变异套件
  `mutations=29 caught=29 control=3 env_limited=0`（32 条样本，baseline `passed=81`）→ PASS；
  `tools/run_all_gates.py` → **12/12 PASS**、棘轮 **39/39**（**现取**，见 `tools/gates-report.txt`）。
  （本行 2026-09-24 交付当时的值是 270 passed / `26-26-3-1` / 10 个门禁；2026-09-25 又现取一次是 11/11；
  同日 B7 还债后再现取是 **13/13、棘轮 2/2**。）
...
- **仍未收口（DoD「产物」行保持未勾）**：只有**一个标的 × 一段 86 行窗口**走完了入库路径 ——
  但按 **B20** 重读，那 86 行**从未提交**（「同连接自读」恒真，见上）；
...
**I3b（I3 之后的一个小步：存储层真接线 + 拦截留痕）收工记录（2026-09-25 实测）**：门禁 **11/11**（`GATES_EXIT=0`，`SCOPE: FULL`）·
棘轮 **39/39**（**未动**：本步没还掉任何 B7 欠账）·
`pytest` **337 passed**（其中本步新增 `tests/test_risk_store.py` **61 条**、`tests/test_backtest_risk_gate.py` §5 **3 条**）。

**为什么它不占迭代号**：本计划 §三 的编号是 I0~I4 **且必须连续**（门禁 C5 核对），
而 **I4 是「绩效看板」**（上一节）。这一小步是 I3 的**收尾加厚**，形态与 I2a / I2b / I2b-3 那些子步相同：
写在 §四 的散文记录里、**不进 §三 表**，所以既不加行、也不破坏编号连续性。
它不出现在 §三 表里 ⇒ **不需要**「已交付 + 证据路径」那一套（C3/C4 只审表里的行）；
正因如此，下面这段的每个数字都必须自己站得住，不能靠门禁兜。
本步的范围取自缺口清单 §七之二的「建议顺序」第 2 项，四条关闭条件逐条对照写在**该节末尾的「交付实测」表**里，不在这里重复。

**真库那一半走的是容器通道（一次性探针，`postgres:17` = 17.11，rc=0）**，并且**这一探就抓到一个真缺陷** ——
本仓库第三次踩「两层各自都绿 ≠ 接起来能跑」：`pgstore.PsycopgConnection.execute()` 把驱动异常收口成
`DataStoreError`（`DATA_008`，**数据中心**错误族）并 `from exc`，于是 `CheckViolation.sqlstate == '23514'`
只存在于 `__cause__` 上；旧实现 `_sqlstate()` 只探本层属性、`except QuanAutoError: raise` 又把它原样放行 ⇒
真库上「库以 CHECK 拒绝 ⇒ RISK_004（旧值继续生效）」这条业务语义**整条失效**：调用方拿到的是 `DATA_008`，
`except RiskError` 接不住，`RiskEngine.apply_change()` 的 `except (RiskConfigLoadError, RiskConfigInvalidError)` 也接不住。
**假连接直接抛裸驱动异常（第一层就探到）⇒ 那条链上的用例全绿而真库不成立**，这正是「对驱动行为的假设属于外部事实」的实例。
修法：放行判据从「是 `QuanAutoError`」收窄到「**是风控族 `RiskError`**」＋沿 `__cause__` 链限深 8 层取 SQLSTATE（`id()` 去环）。
触发测试：`tools/pytest_mutation_check.py` 的**第五套件**（`tests/test_risk_store.py`）新增 11 条变异 + 1 条控制变异，
**39 变异 / 39 CAUGHT + 4 控制变异 + 0 ENV-LIMIT**（基线五套件 `passed=145`，`verdict: PASS`）。
其中 3 条是专门钉上面那条真库缺陷的（放行判据退回 `QuanAutoError` / 不追 `__cause__` 链 / 风控族也被重翻一遍）——
只测裸驱动异常是**抓不到**它们的，这就是为什么假件必须**照抄 `pgstore` 的包装形状**。
两处**效力边界**同样必须连在一起读：① 假连接只能证明「发出去的语句与参数是什么」＋「异常形状怎么翻」，
**不能**证明 PostgreSQL 接受这些语句；② 库端证据来自容器通道（`tools/run_sql_smoke.py` 与 `db/*.sql` 的快照）。
**本步没有改 `db/*.sql`，所以既有的 SQL 冒烟/证伪快照（`tools/sql-smoke-report*.txt`、`tools/falsify-report.txt`）不作废。**
留痕的写入方是**调用方**（`BacktestEngine._record_risk_block()` → `RiskInterceptLogWriter.record()`，`quanauto/engine.py:677`），
契约 §3.6 那句「风控引擎（异步批量写入）」的偏离与理由追加在风控契约文末附录（**只追加，不原地改**）。
证据：`tools/gates-report.txt`、`tools/pytest-mutation-report.txt`、`tests/test_risk_store.py`、`tests/test_backtest_risk_gate.py`。

#### I4 · 绩效看板

| 件 | 具体是什么 | 判定方式 | 证据 |
| --- | --- | --- | --- |
| 契约 | 看板读数的字段清单与口径（取自 `BacktestResult` 与 `PerformanceAnalyzer`，**不得另立一套指标定义**） | 与契约模块二逐字段对齐；实盘实时看板不在本轮 | 核心契约 **§F**（`docs/智能量化交易平台-核心模块接口契约文档.md` 文末追加的 F1~F6） |
| 产物 | 回测绩效看板：读一份 `BacktestResult` → 展示指标表与资金曲线 | 同一份结果文件渲染两次，数值逐位一致 | `quanauto/dashboard.py`、`tests/test_dashboard.py`（§4 断言 `text`/`html` 各渲染两次**逐字节相同**）、`tools/dashboard_failure_demo.py` |
| 门禁 | 看板读数与 `PerformanceAnalyzer` 输出的一致性检查（防两边各算一套指标） | 接入 `run_all_gates.py` 且 tier-A 绿 | `tools/verify_dashboard.py`（新门禁 `dashboard-consistency`）、`tools/gates-report.txt` |
| 触发测试 | 手工改一份 `BacktestResult` 里的某个指标，确认一致性门禁会红 | 一次真实的失败记录 | `tools/dashboard-failure-report.txt`（`ISSUE [C10] sharpe_ratio: the report stores -1.796014 but re-running PerformanceAnalyzer … yields -1.318988…`）、`tools/dashboard_failure_demo.py` |

**I4 收工记录（2026-09-25 实测）**：门禁 **12/12**（`SCOPE: FULL`，新增的 `dashboard-consistency` 已注册）·
棘轮 **39/39**（**未动**：本轮没还掉任何 B7 欠账）· `pytest` **373 passed**（其中本轮新增 `tests/test_dashboard.py` **36 条**）·
变异套件 **49 变异 / 49 CAUGHT + 5 控制变异 + 0 ENV-LIMIT**（基线六套件 `passed=181`）·
`tools/verify_dashboard.py` 本体 `verdict: PASS (0 issue(s))`、`--selftest` **16/16 OK**。

**本轮唯一值得记的一件事：第 1 版的一致性门禁是恒等式，14 个假样本一个都没证明。**
第一版把「一致性」写成「看板读出的数 == 同一份 payload 里的字段」—— 两个数**同源**，
手工改报告会把两边一起改掉，于是永远相等；就算真另立一套指标，也照样绿。
修法是把参照物抬到报告**之外**：C10 拿报告自带的 `trades`/`orders`/`account_history`
**现场重跑真的 `PerformanceAnalyzer`**，再与报告里存的 14 个指标逐字段比（`tools/verify_dashboard.py`）。
**它证明的是「报告是新鲜的、没被手改过」，不是「公式对」** —— C10 的参照物就是写出这些数的同一个
分析器，公式本身错了它看不见（这条边界登记在核心契约 **§F5**）。
第二件：`test_dashboard.py` 的 §6 是**事后补的** —— §1~§5 全绿的时候，`run_dashboard` 一行都没跑过，
而「库里能渲染 + CLI 里能分派」并不等于缝上那几行（读文件 / 选格式 / 落盘 / 分派）能跑
（本仓库已因这个味道吃过三次亏，见 §四 I2b-3 与 `CONTEXT.md`）。
**两处效力边界**：① 看板本轮只有 `text` 与单文件 `html` 两种展示形态，实盘实时看板 / Web 前端 /
图表库 / PDF・Excel 导出都不在本轮；② 看板是**纯读数组件** —— 屏幕上每个数都是 §2.5 的
`PerformanceAnalyzer` 在本次回测里算出来的数，看板一个都不重算（机器判据 = C6 禁止统计模块/函数落进看板）。

**第三件：回填时撞出来的 —— 契约 §F4 不能写看板异常的类名。** 门禁 `core-contract-refs`（tier B）的 T3
判据是「散文里点名的每一个 `*Error` 都必须在**本文档内**有定义」，而核心契约正文的异常层次至今没有枚举
（`tools/gates-baseline.json` 里记着的那笔欠账，T3 = 18）。实测：§F4 写类名 ⇒ 欠账 39 → 40，棘轮当场报
**DRIFT+**；改成按公开**错误码**（`DASHBOARD001`）引用 ⇒ 回到 **39/39**。这里**改的是产物（契约文本）而不是基线**：
棘轮文件自己写明「增长 = 有人引入了新缺陷」，而这一笔是**交付登记**顺手添的欠账，不值得把那条基线从 39 手改成 40。
**这条取舍的代价**：契约正文因此读不出那个类叫什么名字，只有 `quanauto/errors.py` 里有（本节是交付登记，不是待实现规范）。
⚠️ **2026-09-25 追加更正**：上面引用的那个错误码拼写（`DASHBOARD001`）**少一个下划线**，
以实现侧为准应为 `DASHBOARD_001`。本节正文保留原样（它是当次交付的记录，不是规范），
更正记录在核心契约 **`#### G10.`**；这个分歧本身已由门禁 `error-codes` 钉成**显式例外**
（两侧一旦不再分歧就报 `EC-SPELLING-STALE`）。

**同批加固（`tools/verify_md_coverage.py`）**：原先 E5~E8 登记的锚是「裸文字」，只靠 `norm()` 去标记才匹配得上
⇒ 负样本 `str.replace` **删不动它** ⇒ 那四条锚**从来没有真的被测过**，而报告里看不出来。现在控制组逐条断言
「每条锚都必须是文件里的**字面文本**」（删不动就是 HARNESS 级 FAIL），并顺手把 F4（取不到就报错）与 F6（本迭代不做）
也登记为锚；`--selftest` 现报 **14/14**（另两份分母：主文档 3/3、干净样本 `missing=0`）。

证据：`tools/gates-report.txt`、`tools/pytest-mutation-report.txt`、`tools/dashboard-failure-report.txt`、
`docs/智能量化交易平台-核心模块接口契约文档.md`（§F）。

**B7 还债（不占迭代号，2026-09-25 交付）收工记录**：门禁 **13/13**（`SCOPE: FULL`，新增的 `contract-appendix` 已注册）·
棘轮 **2/2**（基线由 39 **手工改小**：`T1=2` / `T2=0` / `T3=0`）·
`tools/verify_contract_refs.py` 的 `--selftest` **10/10 OK**、本体 `verdict: DIRTY (2 finding(s))`（两条全是 `T1`）·
`tools/verify_contract_appendix.py` 本体 `verdict: PASS (0 finding(s))`（`G section: line 2204` / `declarations: 41` / `compared: matched=35 contract-first=6`）、`--selftest` **20/20 OK**（含一条干净控制样本）·
`tools/verify_md_coverage.py --selftest docs` 的 `ADDENDA` 分母由 **14/14 变 19/19**（新增附录 G 的 5 条锚，其中一条是那句
「新副本 ≠ 原文修好」）。

**同批自查（也是这一节里最值得记的一条）**：写完之后回头数了一遍，G7 里「本节登记的 **34** 个实现侧类」
**少算 1** —— 逐类点数与门禁实跑（`declarations: 41` / `matched=35` / `contract-first=6`）对不上，
漏掉的是 `OrderId` 这条 `X = NewType("X", str)` 形状的**声明**（按「类」数就会把它整条掉在括号外）；
G8 里「T1/T2/T3 三类欠账**清零**后基线改小」那句口径也不准（T1 的 2 条是**结构性**的，永远清不了零）。
两句都**没就地改** —— 附录 G 所在文件受「只许追加」管（G8 最后一条写的正是这句）—— 而是追加了一条
**G9 更正**，把真值与口径一并写明；`ADDENDA` 因此再多 2 条锚（分母 **19/19 → 21/21**）。
教训与 §4 的「判据只查单向」同族：**计数必须是数出来的，不能是估出来的** —— 而它最容易在「我刚写完、看起来对」
的那一刻滑过去。

**同批还改掉一句「结论碰巧对、依据是错的」的话（缺口 B8 因此登记）**：数据中心契约 §3.10 有一行写着
「`tools/verify_contract_refs.py` 对本契约审计为 `verdict: CLEAN`」—— 实测**取不到这个值**：
该工具的**位置参数**才是被审文档，`--extra=` 只提供定义、**其正文提及不参与审计** ⇒ 门禁跑的是**主契约**
（`verdict: DIRTY (2 finding(s))`，两条都是主契约自己的结构性 `T1`），**两份手写补充契约自己的提及一条都没被扫过**。
把数据中心契约换成被审文档实测是 `verdict: DIRTY (5 finding(s))`，五条全是 `T3`：`DataStoreError`
（已由该契约附录 C 裁决为「实现侧新增异常码 `DATA_008`」，**不是缺陷**）+ `HTTPError` / `URLError` /
`OperationalError` / `ProgrammingError`（`urllib.error` 与驱动（`psycopg`）所有，本项目**不拥有** ⇒ 审计器缺一份三方白名单）。
那句错话已就地更正并附五条分类；缺口登记为 `docs/开工前缺口清单.md` 的 **B8** —— **不夹带在本步修**，
因为两条修法（把补充契约也当被审文档 / 给工具加三方白名单）**都要动判据本身**，后者还是在放宽检查。
当下能说的话是：门禁绿 = **主契约**的发现数等于基线（`T1=2`）＋ 两份补充契约提供了足以让主契约变干净的定义，
它**不等于**「两份补充契约自身已被审过」。

**改法是「追加附录 G」，不是就地修正文。** 核心契约是 `.docx` 转换产物 ⇒ 只许追加（同 B4 裁决与 §三 I1 的先例），
所以 19 个类型与 18 个异常全部定义在文末的**附录 G** 里，由新门禁 `contract-appendix` 逐字段核对
「附录 G = `quanauto/` 的实现」（基类 / 字段名与顺序 / 类型 / 默认值 / 枚举取值 / 异常 `code`）。
附录 G 还登记了 5 个**契约先行**的名字（正文引用了、本迭代不实现）⇒ 一旦 `quanauto/` 里出现同名实现，
门禁必须报 `AX-PENDING-DRIFT` 把人叫回来改判据 —— 否则那 5 行会从「不实现」变成「静默的旧事实」。
G5 另给了两个不可解析块的**可复制副本**（`AX-T1-COPY` 盯着），因为**剩下那 2 条 `T1` 修不了**：
它们就在 .docx 派生正文里，就地修会被下次重新转换抹掉。这是本仓库第三次遇到「**生成产物只能单向漂移**」
（见 §四 I2b-3 与 `CONTEXT.md` 纪律 10）。

**这一轮还抓到一个真的假绿隐患，值得记住味道**：给 39 个名字补定义时，`tools/verify_contract_refs.py` 把
`OrderId = NewType("OrderId", str)` 报成「未定义」—— 因为它的定义收集器只认 `ast.ClassDef`。
修的时候顺手把 T3（未定义异常）的减法也改成只减「真的类定义」：
**形状别名算 T2 的定义，但绝不能算 T3 的异常定义**。这件事是新增的控制样本
（`T3-newtype-is-not-a-class`：`FooError = NewType("FooError", str)` 必须仍报 `T3`）抓出来的 ——
没有它，这个「修假阳性」的动作会把一个真判据静默放宽成永远绿。

证据：`tools/gates-report.txt`、`tools/gates-baseline.json`、`tools/verify_contract_refs.py`、
`tools/verify_contract_appendix.py`、`docs/智能量化交易平台-核心模块接口契约文档.md`（文末附录 G）。
...
这样 B7 从「一次性 45 处的大工程」变成「可计分的迭代项」—— 它已在 2026-09-25 走到下表末行（**2**）。
...
| **还债（2026-09-25 实测）** | **2** | T1=2 / T2=0 / T3=0 | B7 可还部分还清：19 个类型与 18 个异常由核心契约**附录 G** 定义，新增 `contract-appendix` 逐字段核对；剩下 2 条 `T1` 是**结构性**的（.docx 派生正文只许追加，就地修会被下次重转换抹掉），G5 给的是可复制副本 |
...
- **ROE 取值口径**：保持 `-1..5`。**已结案（2026-09-25 实测）**：契约 §2.3 的表格里 `roe`
  已单列一行、写明「**不受上一行的 0~1 约束** / 区间 `[-1, 5]` / 由 `ck_dc_fin_roe_range` 强制」，
  其后的说明段也点了「别把上表的『比率 0~1』当成所有比率字段的通用规则」这个坑；
  缺口清单 §三 ④ 同批改判。这是文档口径漂移（D3 类），不是新决策。
- **范围**：先只做 phase 1。
