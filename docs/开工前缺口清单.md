...
**工程骨架也已建（2026-09-23，I0 交付）**：`pyproject.toml`（只声明 `requires-python`，
`dependencies = []`）+ `quanauto/` 包（只有版本号，**没有任何实现**）+ `tests/test_skeleton.py`
（实测 `python -m pytest -q` → `3 passed`，退出码 0）+ `.github/workflows/ci.yml` + 第 6 个门禁
`skeleton`。证据：`tools/ci-dryrun-report.txt`（含两次红→绿往返）、`tools/gates-report.txt`。
