...
**工程骨架也已建（2026-09-23 I0 交付；同日晚 I1 在其上打了第一条竖切）**：`pyproject.toml`（只声明 `requires-python`，
`dependencies = []`）+ `quanauto/` 包（I1 已填入 9 个实现模块，入口 `quanauto.cli`）+ `tests/test_skeleton.py`
（3 条骨架自检）+ `tests/test_backtest_slice.py`（22 条竖切回归）+ `.github/workflows/ci.yml` + `skeleton` 门禁。
实测 `pytest` → `25 passed`，退出码 0。证据：`tools/ci-dryrun-report.txt`（含两次红→绿往返）、
`tools/gates-report.txt`、`tools/pytest-mutation-report.txt`。

⇒ 本次改掉的是 B6 的**前半句**（「没有骨架」）。**后半句仍然成立、且必须保留**：
上面那张表的依赖**一个都没装**，`pyproject.toml` 的 `dependencies` 是**故意空的** ——
依赖应当由真正用它的模块带进来，而不是现在按技术选型预置一批「以后大概会用」的包。
