...
> 「端到端回测还没跑通（**一行业务代码都没有**）」**已不成立** —— 仓库现有 14 个模块
> （13 个实现 + 入口 `cli`）与 273 条测试，I1 的端到端回测、I2 的 S1~S3 与 I2a / I2b 各子步、
> I3 的风控介入都已交付。**现状一律以 `CONTEXT.md` §6 与 `docs/迭代计划.md` §三 为准**，
> 不要拿这张表当进度看。
>
> **下一步登记在本文件 §七之二（运行期可观测性）的「建议顺序」里**：第 2 步 = `DbRiskRuleStore`
> 真接线 + `risk_intercept_log` 写入（只追加，SQL CHECK 兜底），关闭条件写在该节末尾。
...
2. **I3 之后一个小步**：`DbRiskRuleStore` 真接线 + `risk_intercept_log` 写入（只追加，SQL CHECK 兜底）。
   验收物应含「拦一次 → 表里多一行，且该行改不动」。
