...
3. **看板错误码的拼写两侧不一致**（2026-09-25 登记）：核心契约附录 **F4**（正文 L2196）**刻意**
   ...
   **没有任何门禁在比对错误码的拼写**（`contract-signature` 比类与成员、`contract-appendix` 比附录 G、
   ...
   待决：在契约附录里追加一条更正，还是把实现侧改成无下划线（后者会与其余全部码的形态不一致）。
   实测口径：`git grep -n "DASHBOARD_001\|DASHBOARD001"` 命中 **4 个文件 7 行**
   （核心契约 L2196/L2198、`quanauto/errors.py` L399/L406、`tools/verify_class_coverage.py` 的登记理由 2 行、
   `docs/迭代计划.md` L509）。
