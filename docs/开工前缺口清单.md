...
| 建议下一步 | 先补 **B7**（主契约缺失类型清单），同时初始化工程骨架并跑通端到端回测；风控层与数据中心已可开工。~~B3（股票池）~~ 已于 2026-09-23 裁决**砍出 MVP**，不再是阻塞项 |
...
另：`d:\project-quan\quan-auto` ~~不是 git 仓库~~ **已修（2026-09-23）**：已是 git 仓库（`main` 分支，基线提交 `053f620`，含 `.gitignore` / `.gitattributes`）。但**仍无工程骨架** —— 无 Python 包结构、无 `pyproject.toml` / `requirements.txt`、无 `pytest`。⇒ C5 只修掉「无提交」这一半，**B6 仍然成立**。
...
> 全仓库仍然零行可运行代码；② 没有端到端的验收物，集成风险全部堆到 Sprint 3 之后。
>
...
6. **工程骨架** — ~~`git init`~~ ✅ **已完成**（2026-09-23，基线提交 `053f620`，含 `.gitignore` / `.gitattributes`）；**剩下**：目录结构、`pyproject.toml`、依赖安装、CI（`pytest`）
...
## 七、需要你决策的问题（2026-09-23：Q1/Q2/Q3/Q4/Q6/Q7 已结案，仅 Q5 待拍板）
...
| 5 | 主文档模块 4 的 `Strategy` 接口是否按契约改齐？ | 建议改齐（契约是强约束）。**此条仍未拍板** |
...
| XML 泄漏 | 扫描 `</?w:` 残留 | 0 处 |
| 换行符 | 检查 CRLF | 0 处 |
| 反向自测 | 截断 md / 构造坏样本，确认门禁真会 FAIL | 5 个探测器全部触发 |
