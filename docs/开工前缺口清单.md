...
> （13 个实现 + 入口 `cli`）与 337 条测试（2026-09-25 I3b 后现取），I1 的端到端回测、I2 的 S1~S3 与 I2a / I2b 各子步、
> I3 的风控介入、I3b 的存储层接线与拦截留痕都已交付。**现状一律以 `CONTEXT.md` §6 与 `docs/迭代计划.md` §三 为准**，
> 不要拿这张表当进度看。
>
> **下一步登记在本文件 §七之二（运行期可观测性）的「建议顺序」里**：第 2 步（**I3b 小步**）=
> `DbRiskRuleStore` 真接线 + `risk_intercept_log` 写入（只追加，SQL CHECK 兜底）已 **2026-09-25 交付**，
> 关闭条件与逐条实测写在该节末尾；**下一个可开工的是建议顺序第 3 步 = I4 绩效看板**。
...
1. `risk_intercept_log` / `risk_decision_log` / `risk_rule_audit` 在 `quanauto/*.py`、`tools/*.py` 里**零引用**；
   ...
   由 `DbRiskRuleStore.save_change()` 写入；**`risk_decision_log` 仍然零引用**。）
