...
### B9. 枚举成员表曾**没有任何判据**（**2026-09-25 发现；B9.1 / B9.2 / B9.3 均已还清**）
...
#### B9.2 类覆盖欠账（✅ 已还清，2026-09-25）

**缺口的形状**：「实现侧某个类有没有契约来源」这件事**没有判据**。当时两条看似相关的门禁都只看
**已登记项的存续**、看不到**新增项**：`tools/verify_contract_signature.py` 核的是「清单里登记的成员
还在不在」（多出来只报 `impl_only` 未登记），`tools/verify_contract_appendix.py` 只审**附录 G 节**。
⇒ 一个新写的类可以既不在契约、也不在任何清单里，而全部门禁照样绿（实测：14 个门禁全绿）。

**新判据**：tier-A 门禁 `class-coverage`（`tools/verify_class_coverage.py`，批内门禁数 14 → 15），
要求 `quanauto/*.py` 里的**每一个类**都落在四个集合之一（前两个自动算，后两个人工登记且**反向自查**）：

| 集合 | 含义 | 本批实测 |
| --- | --- | --- |
| ① `DECLARED` 契约声明 | 三份契约的 ```` ```python ```` 块里由 `ast` 真解出 `class 同名`（**文本搜不算**） | **132** |
| ② `REGISTERED` 登记表 | `manifest.classes` ∪ `impl_only` ∪ `CONTRACT_FIRST` ∪ `T1_REQUIRED`，**并集算出来**（9 / 22 / 6 / 2 ⇒ ∪ 38） | **38** |
| ③ `DEBT` 明账 | 契约**点了名**（含只在 `Raises:` 代码块里出现的）却没有类块 ⇒ 显式欠账 | **7** |
| ④ `LOCAL` 实现细节 | 三份契约**不定义**它（或契约自己已裁决「契约里没有它」） | **22** |

**分母与结果**：`quanauto/*.py` 16 个文件 / **144** 个类（另有 1 个 `NewType` 别名 `OrderId`，不计入类）。
其中 **115** 个由契约声明（101）或登记表（14）直接覆盖；余下 **29** 个（报告里的 `uncovered=29`）
必须被 ③④ 两张表逐个认领，实测 **7 + 22 = 29 全部落表** ⇒ `verdict: PASS (0 finding(s))`。
**没有任何一个类被「提及」豁免**。

- **7 个 `DEBT`**（契约点过名、正文没有类块；每条都写清了契约在哪用了这个名字）：
  `OrderRejectedError`(errors.py:302)、`RiskConfigInvalidError`(errors.py:342)、`RiskConfigLoadError`(errors.py:355)、
  `RiskConfigWriteError`(errors.py:361)、`RiskInterceptError`(errors.py:325)、`RiskRuleNotFoundError`(errors.py:351)、
  `StrategyHandle`(models.py:331)
- **22 个 `LOCAL`**（实现侧细节），其中 **7 个**在契约散文里出现过名字：
  `DataStoreError`(errors.py:255)、`IngestReport`(pgstore.py:184)、`PgBarIngestor`(pgstore.py:373)、
  `PgBarStore`(pgstore.py:352)、`PsycopgConnection`(pgstore.py:433)、`RiskError`(errors.py:319)、
  `RiskInterceptLogWriter`(risk.py:1080)；另外 **15 个**三份契约里连名字都没有：
...
**两张表双向查**（单向判据会让登记表变成永久免检的洞，纪律 10）：表里的名字一旦已能被声明/登记
覆盖 ⇒ `CC-DEBT-DRIFT` / `CC-LOCAL-DRIFT`（免检资格当场作废）；表里的类在实现里消失 ⇒
`CC-DEBT-STALE` / `CC-LOCAL-STALE`（登记失效）。**「提及」与「声明」分开数**（B10 的第二条要求）：
报告里单印一行 `mention split` —— 待裁的 29 个名字里只有 **13** 个在契约散文里出现过、**16** 个连名字都没有；
`CC-UNCOVERED` 的消息里也带提及次数并写死「**提及不算覆盖**」，把「用一句『正文提过』把缺口抹掉」
这条路径在报告里点名。
**豁免也不是免检**：契约里两个被 `.docx` 压坏的块（`Account` / `ResultSerializer`）登记在
`DAMAGED` 表里，但**块里出现新类名照旧报** `CC-UNPARSABLE-CLASS`、**登记项已能解析或类名不在了报**
`CC-DAMAGED-STALE`（豁免不许留成永久洞）。

**证伪（两条通道，都不写仓库）**：

- `--selftest`：**29** 个合成样本，每条断言**预期检查码**（不是「有没有 FINDING」）；含干净样本防误报，
  另有一条「提取为空 ⇒ FAIL」的空转守卫。
- `--falsify`：拿**真实产物的逐字节副本**改坏（先 `shutil.copytree` 到临时目录，并断言副本的 8 个计数
  与真仓库逐一相等，否则报「副本不忠实」）—— 新增一个没人管的类 ⇒ 只报 `CC-UNCOVERED`；
  把 `LOCAL` 登记过的类改名 ⇒ `CC-UNCOVERED` + `CC-LOCAL-STALE`；收尾样本断言拆掉变异后 0 finding
  且计数无漂移。实测 `FALSIFY OK: 3/3 case(s) matched`。

**⚠️ 29 不是基线**：判据要求的是 `uncovered` 里**还有没有被 ③④ 认领的**（即 ③④ 缺一条就红），
不是「不超过 29」—— 29 只是那一次实测的分母。
**边界（它证明不了什么）**：① 只判**方向**（实现侧 ⇒ 必须有来源），反向「契约声明了但实现里没有」
由 `contract-signature` / `contract-appendix` 管，本门禁只**印**计数；② 只数类，不比成员/字段/方法；
③ **不判「某个类该不该有契约块」** —— `StrategyHandle` 该补契约块还是该改返回值是人的裁决，
扫描器只保证「两侧不一致时一定有人知道」。

> **计数更正（2026-09-25 二次实测）**：本节早先的 **133 / 147 / 32 / 14+15** 四个数都不准。
> 用 `ast`（三份契约的 ```` ```python ```` 块里数 `ClassDef`）与独立的正则文本搜
> （`^[ \t]*class[ \t]+(\w+)`，先 `utf-8-sig` + CRLF 规范化）两种法子重数，**都是 132**。
> 但有一种更松的写法会数出 **133**：`^[ \t]*(?:class|@dataclass)[ \t]+(\w+)` —— 它多出来的那个
> 「名字」是**字符串 `class` 本身**，来自核心契约 **L1168** 那条被 `.docx` 压坏的行
> `@dataclass class Account: account_id: str # 账户ID total_capital: float …`：
> 正则先匹配 `@dataclass`，再把紧跟其后的关键字 `class` 当成了类名。
> ⇒ 133 = 132 个真名字 + 1 个假名字；**并集 147 → 146**（`声明 ∪ 登记`）、**「声明了但实现里没有」32 → 31**。
> 同一处还有一次「提及」口径分歧：`RiskRuleNotFoundError` 只出现在风控契约 **L380 的 ```` ```python ```` 块**里
> （`RiskRuleNotFoundError: GLOBAL 层缺失该规则时抛出。`），**散文里一次都没写** ⇒ 严格口径下它不算
> 「正文提到过」，于是 **14 MENTIONED / 15 ABSENT → 13 / 16**（缺口的**总数 29 不变**，只是它在
> `DEBT` 那一栏而不是 `LOCAL`）。
> **计数更正（本批更早一次）**：早先写的「145 个类」也是错的，实测 **144** —— 那 145 来自门禁
> `contract-appendix` 报告里 `impl scan: … classes=145`，它把 `NewType` 别名
> （`quanauto/models.py:51` 的 `OrderId`）也收进了比对表（那是它的设计：附录 G 里 `OrderId` 确实要逐字段比），
> 但**标签不该写 `classes=`**；2026-09-25 已改成 `classes=144 newtypes=1` 两个数分别打印。
> 教训同上一次：**更正里写下的新句子同样要实测** —— 上面这三个错数全都是「看起来更精确」的写法带来的。
...
### B10. 「实现侧某个类/枚举有契约来源」应当是一条**由并集算出来的**判据（✅ 已还清，2026-09-25）
...
**落实（2026-09-25，与本条三条要求逐条对应）**：

1. **两侧都由并集产生、不写死数字**：`tools/verify_class_coverage.py` 的声明侧只认 `ast` 解出的
   `ClassDef`（文本搜不算），登记侧是 `manifest.classes ∪ impl_only ∪ CONTRACT_FIRST ∪ T1_REQUIRED`
   的**并集**（实测 38）；两侧都有下限守卫（`MIN_DECLARED=80` / `MIN_REGISTERED=25` / `MIN_IMPL=100`），
   提取变薄即报 `CC-THIN-*`，提取为空另有专门样本 ⇒ 不会打「0 问题 PASS」。
2. **「提及」与「声明」分开数**：报告单印一行 `mention split`（29 个待裁名字：散文提到 13 / 连名字都没有 16），
   `CC-UNCOVERED` 的消息里带提及次数并写死「提及不算覆盖」——`RiskRuleNotFoundError` 就是这条口径的实测样本
   （它只在 `Raises:` 代码块里出现过，散文一次都没写）。
3. **正反向都有负样本，且断言预期检查码**：`--selftest` 29 个样本逐条断言检查码；`--falsify` 在真实产物的
   逐字节副本上做两个方向（新增无人管的类 ⇒ `CC-UNCOVERED`；登记的类被改名 ⇒ `CC-LOCAL-STALE` + `CC-UNCOVERED`），
   收尾断言拆掉变异后计数无漂移。「新增一个未登记的**枚举成员**会报错」这一支由 `enum-members` 覆盖。

**当下能说的话**：枚举成员这一层有判据（`enum-members`，含 24 个自测样本，两个干净样本防误报）；
**类这一层现在也有**（`class-coverage`，含 29 个自测样本 + 3 条真实产物证伪）。**但不要说「实现侧都有契约来源」** ——
准确的说法是「**每一个类都有人认领过**」：115 个由契约声明/登记表覆盖，另外 29 个分别在 `DEBT`（7，契约点了名
却没形状，是明账欠款）与 `LOCAL`（22，登记为实现侧细节）里。**门禁绿不等于实现对**，它只保证「多出一个
没人管的类时会有人知道」。
...
3. **看板错误码的拼写两侧不一致**（2026-09-25 登记）：核心契约附录 **F4**（正文 L2196）**刻意**
   按公开错误码引用看板异常，写的是 `DASHBOARD001`；实现 `quanauto/errors.py`（L406）的 `code`
   是 `DASHBOARD_001`。同族其它码都带下划线（`RISK_004` / `DATA_008` / `BROKER_002`）⇒
   契约那一处像是漏了下划线，但**契约正文是 `.docx` 派生、只许追加**，不能就地改。
   **没有任何门禁在比对错误码的拼写**（`contract-signature` 比类与成员、`contract-appendix` 比附录 G、
   `enum-members` 比枚举成员、`class-coverage` 比类的归属）⇒ 这个分歧只能靠本节留痕。
   待决：~~在契约附录里追加一条更正，还是把实现侧改成无下划线~~
   **已裁决（2026-09-25）：追加更正，以实现侧为准。** 实现侧不改 —— 同族全部码都是
   `<前缀>_<三位数字>` 的形状，为一个孤立拼写破形会把这个形状打散。更正落在核心契约
   **`#### G10.`**（L2717，追加在 G9 之后），并由 `tools/verify_md_coverage.py` 的 `ADDENDA`
   登记 —— 被下一次重新转换抹掉就报 `addenda-missing`。
   **这条分歧现在有机器判据了**：门禁 `error-codes`（`tools/verify_error_codes.py`，2026-09-25 注册，
   tier A）把三份契约的错误码表与 `quanauto/errors.py` 在**码 / 拼写 / 级别 / 类名**上对拍，
   并把它登记成**显式例外** —— 一旦两侧不再分歧（有人把 F4 原文改了），报 `EC-SPELLING-STALE`
   「免得例外长成永久免检的挡箭牌」。**同一轮这条门禁首跑还抓到另一条真冲突**，
   见下文「门禁 `error-codes` 首跑抓出的真冲突」。
   实测口径（2026-09-25 重测）：`git grep -n "DASHBOARD_001\|DASHBOARD001"` 命中
   **6 个文件 16 行**（核心契约 L2196/L2198/L2723~L2726、`quanauto/errors.py` L399/L406、
   `tools/verify_class_coverage.py` 2 行、`tools/verify_md_coverage.py` 2 行、`docs/迭代计划.md` L509、
   本节 3 行）；把**未跟踪**的 `tools/verify_error_codes.py`（15 行）也算上，工作树里是
   **7 个文件 31 行**。原文写的是「**4 个文件 7 行**」—— 那个数在当时是实测，现已过期。

**附带澄清（免得被当成缺陷）**：`numeric` 列回读成字符串时可能是科学计数法（实测 `1.0E-7`），
这是驱动的正常表示、不是丢精度；审计表因此刻意用 `varchar(64)` ＋ `_audit_num()` 的定点写法，
让 D6 的「变更前后可读」真的可读。

### 门禁 `error-codes` 首跑抓出的真冲突（2026-09-25，**已修**）

> 这一条**不是本次探针的产出**（探针那三条在上面），它是新门禁**第一次拿真实产物跑**就抓到的 ——
> 也正是「先拿真实产物验一次新判据」这件事的价值所在。

`RISK_001` 的级别在两份契约里不一致：核心契约**附录 D** 写 `CRITICAL`，风控契约 **§3.7** 写
`ERROR`，而 §3.7 那一行还标着「（主契约原有）」—— 转抄抄错了。两处「各自都能自洽」了很久，
因为**没有任何判据在比级别**（上面第 3 条列的那四条都不看错误码）。

裁决依据两条：① 主契约是 `RISK_001` / `RISK_002` 级别的权威，而 §3.7 自己那句
「两处定义必须保持一致，不得冲突」本身就是一条**没有判据的要求**；
② `quanauto/risk.py` 的 severity→动作映射是「`CRITICAL` ⇒ 至少 `REJECT`」，而 REJECT/HALT 抛的
正是 `RISK_001` ⇒ 实现侧的行为支持 `CRITICAL`。

已**改产物**（风控契约 §3.7 那一行改回 `CRITICAL`，并在表后留一条 2026-09-25 更正块说明它原先为何写成 `ERROR`），
**没有削弱判据**。那句话从 2026-09-25 起由 `EC-LEVEL-CONFLICT` 兜着（跨契约共用同一个码时才互查）。
