...
> （这条要求此前**没有任何机器判据**；自 2026-09-25 起由门禁 `error-codes`（`tools/verify_error_codes.py`）核对 —— 它首次就跑出了 `RISK_001` 的级别冲突，见下表后的更正。）

| 错误码 | 级别 | 描述 | 建议措施 |
| --- | --- | --- | --- |
| `RISK_001` | CRITICAL | 风控拦截 | 检查风控规则和仓位管理（主契约原有） |
...
> **2026-09-25 更正**：上表 `RISK_001` 原写 `ERROR`，与主契约附录 D 的错误码表（`CRITICAL`）
> 冲突 —— 而这一行本来就标着「（主契约原有）」，等于转抄时抄错了。**主契约是 `RISK_001` /
> `RISK_002` 级别的权威**（上面那句衔接说明也是这么写的），已按它更正。旁证在实现侧：
> `quanauto/risk.py` 的严重度-动作映射是「`CRITICAL` ⇒ 至少 `REJECT`」，而 `RISK_001` 正是
> 裁决为 REJECT/HALT 时抛出的（见 `quanauto/errors.py` 里那个类的 docstring）。这次不一致是
> 门禁 `error-codes` 的第一次实跑报出来的 —— 也就是说，上面那句「不得冲突」现在真的有人盯。
...
- **仍然没接线的部分**（写在明处，免得被当成已完成）：`emergency_flatten()` 仍只返回任务标识（真正的清仓执行不在本仓库的 I0~I4 任何一轮）；
  短轮询 watchdog 只在 DB 版才需要，回测路径不启动；**`risk_decision_log` 仍然零写入者** —— 指**产品写入路径**：
  `quanauto/*.py` 里一处都没有（静态核对器 `tools/verify_risk_config.py` 只在表清单与约束表里**引用**它的名字，
  引用 ≠ 写入；`db/risk_control.smoke.sql` 里那两条 `INSERT` 是**约束触测**，不是采集/下路路径的一部分）。
...
- **门禁**：`python tools/run_all_gates.py` → **11/11**（`GATES_EXIT=0`，`SCOPE: FULL`）；B7 棘轮 **39/39 未动**（本步没有还掉任何欠账）。
  **（本行是 2026-09-25 本步的实测快照，不改。**当天稍晚 B7 还债后这两个数都变了：门禁 **13/13**、
  棘轮基线由 39 手工改小到 **2**（`T1=2` / `T2=0` / `T3=0`）—— 见核心契约**附录 G9** 与 `docs/迭代计划.md`
  的「B7 还债收工记录」。读本行时请连日期一起读；**要现值请现取** `tools/gates-report.txt`。）
